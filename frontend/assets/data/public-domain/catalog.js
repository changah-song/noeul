...
// Generated by backend/scripts/generate_catalog_levels.py — the SAME pipeline
// the backend uses for chapter preprocessing (kiwipiepy / jieba +
// proficiency_levels.db), run over each book's FULL bundled text. Unlike the
// old hand-rolled 100-word samples, these carry the per-band `distribution`
// that bookEaseFromDistribution needs for a continuous reading-ease estimate.
// Re-run the script after adding or editing bundled book texts.
const GENERATED_LEVELS = require('./levels.generated.json');

const PUBLIC_DOMAIN_LEVELS = Object.keys(GENERATED_LEVELS).reduce((levels, id) => {
  const bookLevel = GENERATED_LEVELS[id];
  levels[id] = { difficulty: bookLevel.level, bookLevel };
  return levels;
}, {});
